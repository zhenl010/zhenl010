...
struct Vertex
{
    ...
    bool operator<(const Vertex& rho) const
    {
        return (this->y_ * 100000 + this->x_) < (rho.y_ * 100000 + rho.x_);
    }
};
...
void RawSegmentToGraph(std::map<IntVector2, int>& vertices, std::vector<RawEdge>& edges,
    const std::list<RawEdge>& rawEdges)
{
    using namespace std;
...
void GenerateGraph( std::vector<Vertex> &allVertices, std::vector<Edge> &allEdges,
    std::map<IntVector2, int>& vertices, 
    const std::vector<RawEdge>& edges)
{
    ...
    // add all vertices
    unsigned int idx = 0;
    for (std::map<IntVector2, int>::iterator itov=vertices.begin(); itov!=vertices.end(); ++itov)
    {
    ...
    std::vector<Vertex> &allVertices, int cnum)
{
    ...
    typedef std::map<Vertex, size_t> VexHash;
...
int PenLift::numTimes(std::vector <std::string> segments, int repeatNum)
{
    ...
    std::map<IntVector2, int> rawVertices;
